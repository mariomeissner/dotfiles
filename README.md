...
The macOS defaults and Dock hooks restart Dock after changing preferences or
items. They verify the `com.apple.Dock.agent` launch service comes back and
kickstart it if launchd leaves it stopped.
...
- Claude Code: installed with Anthropic's native installer instead of Homebrew
  ...
  live under `~/.claude/`. This repo does not manage `~/.claude` with chezmoi;
  use `sh scripts/seed-claude.sh` for a conservative new-machine seed.
- Herdr, Codex CLI, Grok CLI, and OMP: installed with their official standalone
  installers. Their binaries live in `~/.local/bin`, ahead of application or
  package-manager copies on `PATH`. The installer hook bootstraps a missing
  binary but does not update an existing installation. The wrappers suppress
  installer-generated shell startup blocks because this repo exclusively owns
  `~/.zprofile` and `~/.zshrc`. Use `herdr update`, `codex update`,
  `grok update`, and `omp update` for their ongoing updates.
- Coding-agent CLI home directories are app state, not chezmoi targets. This
  applies to Claude Code, Codex, Grok, OMP, and similar tools added later. Keep
  their settings, memory, skills, plugins, histories, hooks, and generated
  integration files out of managed dotfiles; use explicit seed scripts for
  first-machine setup instead.
- Herdr integrations: generated by Herdr itself, not vendored in this repo.
  Run `herdr update`, then `herdr integration install claude` and
  `herdr integration install codex` after Herdr reports outdated integrations.
  For OMP, run `herdr integration install omp`.
  Files such as `~/.claude/hooks/herdr-agent-state.sh`,
  `~/.codex/herdr-agent-state.sh`, and `~/.codex/hooks.json` are Herdr-owned
  generated files.
